...
{/*Fetch DELETE */}
  ...
  const confermaEliminazione = () => {
    fetch(`https://teenage-hynda-mattiazerbini-e6e83c07.koyeb.app/servizi/${servizioEliminato}`, {
      method: "DELETE",
      headers: {
        Authorization: `Bearer ${token}`,
      },
...
const getServizioId = async(id)=>{
  const result = await fetch(`https://teenage-hynda-mattiazerbini-e6e83c07.koyeb.app/servizi/${id}`, {
      headers: {
        Authorization: `Bearer ${token}`},
      });
      return result.json();
...
const handleUploadImg = async (id) => {
  ...
  try{
    const result = await fetch(`https://teenage-hynda-mattiazerbini-e6e83c07.koyeb.app/servizi/${id}/immagine`, {
      method: "PATCH",
