...
export default function CampiPadel(){
  ...
  const confermaEliminazione = () => {
    fetch(`http://unable-kalie-mattiazerbini-d54da76b.koyeb.app/campi/${campoEliminato}`, {
      method: "DELETE",
      headers: {
        Authorization: `Bearer ${token}`,
      },
...
const handleAttivo = async(id, stato) =>{
  ...
  try{
    const result = await fetch(`http://unable-kalie-mattiazerbini-d54da76b.koyeb.app/campi/${id}/attivo`, {
method : "PATCH",
headers: {
  "Content-Type" : "application/json",
   Authorization: `Bearer ${token}`,
...
const handleUploadImg = async (id) => {
  ...
  try{
    const result = await fetch(`http://unable-kalie-mattiazerbini-d54da76b.koyeb.app/campi/${id}/immagine`, {
      method: "PATCH",
